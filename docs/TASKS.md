...
## Phase 22 — 스트림 회수 + 네이티브 SSE 파서 (브랜치 `claude/t149-stream-recovery`)

| ID | 작업 | 파일 | 상태 |
|----|------|------|------|
| T-149 | 채팅 스트림이 도중에 끊긴 뒤에도 서버가 저장한 답을 회수한다(핵심 회귀: 2026-08-05 17:55 KST 사례 — aiohttp `ClientConnectionResetError` 4초 뒤 게이트웨이가 state.db에 답 저장했지만 앱은 즉시 `[에러]` 버블을 붙여 답이 화면에 안 뜸). ①`send()` catch에서 무조건 `[에러]`를 append하던 것을 제거하고 대기 어시스턴트 버블을 do-scope 바깥으로 승격 — **좁은 화이트리스트(networkConnectionLost/timedOut, 조기 EOF 포함)에 걸리는 전송 오류일 때만** T-116의 `pollForMissedReply` 앵커링(마지막 user 뒤 visible assistant)으로 회수 시도, 성공하면 대기 버블에 채우고 실패했을 때만 **원본 오류를 그대로** 에러 버블로 대체(타입 보존). TTS/알림/자동 제목은 정확히 한 번씩 실행. ②사용자 중지는 두 경로(`CancellationError` throw / `URLError.cancelled`를 클라이언트가 finish로 승격)에서 모두 회수·에러 없이 조용히 종료 — `Task.isCancelled` 가드로 300초 폴링 방지. ③**`SSEParser`가 프로토콜-종료(`done`/`[DONE]`)와 실행-종료(`run.completed`)를 분리** (수용 기준): `run.completed`는 `sawRunCompleted` 플래그만 세우고 파이프를 살려둔다 — 뒤이어 오는 `done`을 놓치지 않도록. 바이트 EOF가 `run.completed` 없이 오면 `HermesAPIClient`가 `URLError(.networkConnectionLost)`로 승격해 상위 회수 폴링을 유발한다. `run.completed` 뒤 EOF는 정상 종료. ④**도구 이벤트 안전 병합** (수용 기준): 실제 게이트웨이 payload의 `message_id`는 **턴 전체** 식별자라서 그걸 도구 ID로 쓰면 서로 다른 도구가 하나로 합쳐진다. 파서가 tool.started마다 별도 합성 ID를 부여하고 같은 이름의 completed/failed에 FIFO 매칭 — 같은 턴에 서로 다른 도구/반복 같은 도구 모두 개별 카운트. deployed(`{message_id,run_id,tool_name,tool_args,sequence}`)와 source(`{message_id,run_id,tool_name,args,seq}`) 두 스키마 모두 수용, 문자열·객체 args 모두 처리. ⑤**에러 타입 보존** (수용 기준): `HermesAPIClient.streamChat`이 `HermesAPIError`(401/serverError/파서 에러)를 별도 catch로 원형 전파, URLSession 전송 오류만 `.network(...)`로 감쌈. `ChatViewModel.isRecoverableTransportError`가 `.network(URLError.networkConnectionLost|.timedOut)`만 통과시켜 명시적 서버/인증/파서 오류는 즉시 표면화(폴링 안 함). ⑥**배열 안전성** (수용 기준): 자리표시자 어시스턴트 버블을 `Int` 인덱스가 아니라 `UUID` id로 추적 — await 사이 배열이 흔들려도 매번 `firstIndex(where:)`로 재해석하고 사라졌으면 조용히 건너뛴다(subscript 크래시 방지). ⑦`recoveryDeadline`은 nonisolated static — 끊긴 스트림+빈 본문 300s, 끊긴 스트림+부분 본문 30s, 정상 종료+빈 본문 300s(T-116), 정상 종료+think-only 6s. ⑧회귀 테스트 `HermesChatTests/SSEParserTests.swift`·`ChatRecoveryTests.swift`(신규) — 델타 중복 없음, 완성본 fallback, 도구 FIFO 병합(다른 도구/반복 도구 개별 카운트), `run.completed`는 종료 안 함/`done`은 종료, 두 payload 스키마 수용, error 이벤트 표면화, 레거시 청크, `missedReply` 앵커링, `isRecoverableTransportError` 화이트리스트, `recoveryDeadline` 규칙. **Linux 개발 환경에는 Swift/Xcode toolchain이 없어 `xcodebuild`/XCTest 미실행 — 정적 검사만 수행. HermesChatTests 유닛 테스트 타깃이 pbxproj에 없고, 멀티 에이전트 환경에서 pbxproj 수기 편집은 파손 위험이 커서 이 커밋에서는 pbxproj를 건드리지 않는다** — 맥에서 Xcode `File → New → Target → iOS Unit Testing Bundle`로 타깃 생성 후 이 파일들을 Compile Sources에 넣고 `xcodebuild test`. 실기기 시나리오: 채팅 중 폰을 비행기 모드로 잠깐 껐다 켜 SSE 연결을 리셋 → 서버가 답 저장 시 회수돼 `[에러]` 없이 답이 뜨고 TTS 재생·자동 제목 1회 실행. 중지 버튼 즉시 종료(회수 폴링 없음). 게이트웨이 재시작·인증 실패 등 서버가 실패를 명시할 땐 회수 없이 원본 오류가 즉시 표면화 | `HermesChat/Services/HermesAPIClient.swift`, `HermesChat/StreamModels.swift`, `HermesChat/ViewModels/ChatViewModel.swift`, `HermesChatTests/SSEParserTests.swift` (신규·테스트 타깃 등록 대기), `HermesChatTests/ChatRecoveryTests.swift` (신규·테스트 타깃 등록 대기), `HermesChatTests/README.md` (신규) | NEEDS-BUILD (2026-08-05, iOS Simulator BUILD SUCCEEDED @ 2ea79f9; 실기기 스트림 리셋 시나리오·테스트 타깃 등록 대기) |
...
| 08-05 | claude/t149-stream-recovery @ 2ea79f9 | BUILD SUCCEEDED | Hermes가 Mac Xcode 26.3에서 iOS Simulator 빌드 검증. T-149 실기기 스트림 리셋 시나리오와 XCTest 타깃 등록은 대기. |
