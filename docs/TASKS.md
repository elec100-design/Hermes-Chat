...
| T-168 | **Live Hermes 세션 제목 유일화(400 invalid_title) + async 경고 제거** — 실기기 검증에서 발견: 게이트웨이가 세션 제목 전역 유일성을 요구해 같은 첫 마디("안녕")로 시작하면 400. 제목에 초 단위 시각을 붙여 유일화 + 실패 시 제목 없이 1회 재시도(서버 자동 제목)로 대화가 절대 막히지 않게. `ChatViewModel.sendDemoResponse`의 불필요한 `await DemoData.nextResponse()` 경고 제거 | `Services/HermesLiveService.swift`, `ViewModels/ChatViewModel.swift` | NEEDS-BUILD (2026-07-04) — 실기기: Hermes 통화 반복 시작 시 400 없음 + 빌드 경고 0 |
| T-169 | **사진 첨부 업로드 타임아웃 해소** — 실사용(safety 프로필)에서 사진 업로드가 시간초과로 실패. 원인 3개: (1) `BridgeClient.upload`이 기본 15초 타임아웃 사용 → 전용 세션(요청 120s/리소스 600s, `waitsForConnectivity`)+타임아웃·연결끊김 1회 재시도, 본문은 `upload(for:from:)` 스트리밍. (2) 아이폰 원본 사진이 5~15MB로 그대로 전송됨 → 긴 변 2048px·JPEG 0.8 축소(HEIC 변환 로직 확장, GIF·작은 이미지 제외, PNG 알파는 흰 배경 평탄화). (3) **Bridge**: 401/404(미등록 프로필)/400을 본문 읽기 전에 응답해 클라이언트가 남은 MB를 쓰다 broken pipe → 타임아웃으로 보임. `fail()`이 응답 전 본문을 drain + `read_body` 짧은 읽기 루프 보강(대용량 잘림 방지). 로컬 재현 검증: 구버전 `BrokenPipeError` → 신버전 정상 404 | `Services/BridgeClient.swift`, `ViewModels/ChatViewModel.swift`, `server/hermes_bridge.py` | NEEDS-BUILD (2026-07-27) — **맥미니 브리지 재배포 필수** + 실기기: 대용량 사진 첨부 전송 성공 |
| T-170 | **Bridge 인증 실패 진단** — 실기기 스크린샷에서 safety 프로필 모델·SOUL이 둘 다 "API Key가 유효하지 않습니다"로 실패(=브리지 401). 실제 원인은 게이트웨이 API Key가 아니라 **Bridge Token 불일치**인데 메시지가 뭉뚱그려져 엉뚱한 키를 고치게 됐다. `HermesAPIError.bridgeUnauthorized` 분리(브리지 401 전용 안내), 설정 → Hermes Bridge에 **연결 테스트** 추가(1단계 `/health` 무인증=주소·서비스 식별, 2단계 `/profiles` 인증=토큰 판정, 토큰 비었으면 그 사실을 명시), 브리지 `/health`에 `auth_required` 노출(토큰 값은 미노출), `BridgeClient.health()` 추가 | `StreamModels.swift`, `Services/BridgeClient.swift`, `Views/SettingsView.swift`, `Resources/*.lproj/Localizable.strings`, `server/hermes_bridge.py` | NEEDS-BUILD (2026-07-27) — **맥미니 브리지 재배포 필요**(auth_required) + 실기기: 연결 테스트가 원인을 정확히 지목하는지 |
