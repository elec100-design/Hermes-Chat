...
### Phase 14 — Deep think 멀티 에이전트 토론룸 (2026-06-11 구현)
...
(제목 `[Deep think] <주제>`, system_prompt로 토론 규칙 주입) 후 앱이 순차로 발언을 중계.
라운드 1은 같은 라운드 선발언자 발언 포함, 라운드 k>1은 타인의 최신 발언만 전달
(자기 발언은 자기 세션 히스토리에 이미 있음 — 토큰 절약). 발언은 `strippingThink` 후
시스템 프롬프트로 간결 강제(6문장). 도구 사용은 설정 토글(기본 OFF — 켜면 한 턴이 수 분
가능 경고). 라운드 수 1~5 선택(기본 2), 사회자 지정 가능(기본 첫 참가자, 탈락 시 승계).
게이트웨이 오류 참가자는 탈락 처리 후 계속, 활성 2명 미만이면 중단. 완료 토론은
UserDefaults JSON 보관(최대 20건, 지난 토론 재열람). 진행 중 `isIdleTimerDisabled`로
화면 꺼짐(앱 suspend → 토론 중단) 방지. 진입은 fullScreenCover — push는 엣지 스와이프
실수로 장시간 토론이 소실될 수 있어 배제.
